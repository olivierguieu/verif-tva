...
{    
    ...
    countryNamesByCode = [[NSDictionary alloc] initWithContentsOfFile:plistPath];
    ...
    NSMutableArray *names = [[NSMutableArray alloc] initWithCapacity:[countryNamesByCode count]];
    for (NSString *code in [countryNamesByCode allKeys])
    {
        [names addObject:[[countryNamesByCode objectForKey:code] objectAtIndex:0]];
    }
    ...
    {
        // tambouille pour extraire le "AT-..."
        NSArray *chunks= [name componentsSeparatedByString: @"-"];
        NSLog(@"%@", [chunks objectAtIndex:0]);

//        KO ??
//        NSString *code = [countryCodesByName objectForKey:[chunks objectAtIndex:0]];
//        [codes addObject:code];
        [codes addObject:[chunks objectAtIndex:0]];
    }
